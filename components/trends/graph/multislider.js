export const getDateMultislider = (mlp) => {
  const Multislider = mlp.createClass(mlp.Observable, {
    initialize: function (options) {
      ...
      let container = mlp.html(
        `<div style="width:100%; height:${containerHeight}px; position:relative"></div>`
      );
      self.dom = container;

      let background = mlp.html(
        `<div class="slider-background" style="width:calc(100% - ${self.handleSize}px); height:${threadHeight}px; position:absolute; top:0; bottom:0; margin:auto;"></div>`
      );
      ...
      for (let i = 0; i < self.handlesConfig.length - 1; i++) {
        let edge = mlp.html(
          '<div class="slider-edge" style="position:absolute; top:0; bottom:0; margin:auto"></div>'
        );
        edge.style.height = background.style.height;
        edge.style.borderRadius = background.style.borderRadius;
        edge.style.backgroundColor = self.edgesConfig
          ? self.edgesConfig[i].color
          : "";
      ...
      for (let handleConfig of self.handlesConfig) {
        let handle = mlp.html(
          '<div class="slider-handle" style="position:absolute"></div>'
        );
        ...
        if (handleConfig.background)
          handle.style.background = handleConfig.background;
      ...
      for (let handle of self.handles) {
        ...
        handle.addEventListener(
          "touchstart",
          mouseToTouchHandler(onHandlePress),
          { passive: false }
        );
      }
    ...
    _repositionHandles: function () {
      for (let handle of this.handles) {
        handle.style.left =
          this.valueToHandlePosition(handle.dataset.value) + "px";
      }
    ...
    _repositionEdges: function () {
      for (let i = 0; i < this.handles.length - 1; i++) {
        ...
        let b =
          parseFloat(this.handles[i + 1].style.left) + this.handleSize / 2;
    ...
    setValues: function (newValues) {
      for (
        let i = 0;
        i < Math.min(this.handles.length, newValues.length);
        i++
      ) {
    ...
    valueToHandlePosition: function (v) {
      ...
      let p =
        this.handleSize / 2 + a * (this.dom.clientWidth - this.handleSize);
      p -= this.handleSize / 2;
      return p;
    },
    ...
    handlePositionToValue: function (p) {
      p += this.handleSize / 2;
      let a =
        (p - this.handleSize / 2) / (this.dom.clientWidth - this.handleSize);
      let v = this.min + a * (this.max - this.min);
      return v;
    },
  });

  const DateMultislider = mlp.createClass(Multislider, {
    initialize: function (options) {
      if (options.handles) {
